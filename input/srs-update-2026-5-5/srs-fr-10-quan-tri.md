...
**Số FR:** 27 (gốc 25 + FR-VIII-26 Quên MK/Kích hoạt + FR-VIII-28 Nhật ký HT `[GAP-VIII-02]` + FR-VIII-29 Quản lý ngày lễ `[GAP-VIII-05]`; FR-VIII-06 đã chuyển sang FR-04)
...
| 2026-05-06 | BA + SRS Agent | **Phát hành v3.5**: gói 14 Thay đổi từ delta report `v3.5-delta-fr-10.md` + 5 fix V4-CHƯA-SỬA (C.1 SCR-VIII-03 row 20 mật khẩu thêm "ký tự đặc biệt"; C.2 §6 Tổng quan BR cột "Áp dụng FR" mở rộng cho FR mới 22-29; C.3 SM-TAIKHOAN "Tham chiếu FR" sửa thành FR-VIII-15, FR-VIII-20-22, FR-VIII-26; C.4 SM bảng chuyển trạng thái dòng [*] → CHO_KICH_HOAT đổi FR Ref FR-VIII-18 → FR-VIII-15; C.5 SCR-VIII-03 cột Hành động thêm nút "Phân quyền" cho TK CHO_PHAN_QUYEN). |
...
| 7 | Tạo bản ghi TAI_KHOAN ở trạng thái CHO_KICH_HOAT, gán vai trò "DN" sẵn (KHÔNG qua trạng thái CHO_PHAN_QUYEN — vì vai trò DN đã rõ, không cần QTHT phân quyền). Set `username = ma_so_thue`, `email = email DN khai` | SM-TAIKHOAN, BR-AUTH-USERNAME-01 |
...
**Acceptance Criteria:**
- **Given** DN truy cập màn hình đăng nhập **When** bấm button "Đăng ký (dành cho doanh nghiệp)" **Then** form đăng ký mở với 21 trường nhập (19 trường thông tin DN + 2 trường mật khẩu) + 1 ô cam kết. Username hiển thị readonly = mã số thuế.
...
| 16 | content | Cột Hành động | button | Xem / Sửa / **Phân quyền** (khi trạng thái = CHO_PHAN_QUYEN: gán vai trò + đơn vị → chuyển HOAT_DONG) / Mở khóa / Khóa / Gửi lại email / Đổi MK | click → hành động tương ứng | tùy trạng thái |
...
|----|-----|---------|-------|--------|--------|--------|
| [*] | CHO_KICH_HOAT | QTHT tạo tài khoản / User đăng ký | — | Gửi email kích hoạt | FR-VIII-15, FR-VIII-22 | — |
