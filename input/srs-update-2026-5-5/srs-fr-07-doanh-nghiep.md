...
**Priority:** Essential
**Stability:** High
**Màn hình:** SCR-V.III-01 — [Danh sách Doanh nghiệp](#scr-v-iii-01-danh-sách-doanh-nghiệp), SCR-V.III-02 — [Chi tiết DN](#scr-v-iii-02-thêm--chi-tiết-dn)
...
**Thêm mới:** **BỎ** — DN được tạo qua self-registration (FR-VIII-22 ở srs-fr-10), CB Nghiệp vụ không tạo DN trực tiếp. CB Nghiệp vụ muốn tạo vụ việc cho DN nào → DN đó phải có TK trong hệ thống trước (do DN tự đăng ký).
...
**Xuất Excel:** `[GAP-V.III-01]`

| Bước | Mô tả xử lý | BR áp dụng |
|------|-------------|-----------|
| 1 | Kiểm tra quyền CB NV | BR-AUTH-01 |
| 2 | Áp dụng filter hiện tại (tên DN, mã số thuế, loại hình, địa bàn) | — |
| 3 | Truy vấn DOANH_NGHIEP theo filter | — |
| 4 | Kiểm tra kết quả ≤ 10.000 dòng; nếu vượt → trả lỗi ERR-DN-04 | — |
| 5 | Tạo file .xlsx với dữ liệu đã lọc | — |
| 6 | Trả file download cho user | — |
...
| E5 | Xuất Excel > 10.000 dòng `[GAP-V.III-01]` | ERR-DN-04 | "Kết quả vượt 10.000 dòng. Vui lòng thu hẹp bộ lọc" | WARNING |
...
- **Given** CB NV xem danh sách DN **When** nhấn "Xuất Excel" **Then** tải file .xlsx chứa danh sách DN theo filter hiện tại (max 10.000 dòng) `[GAP-V.III-01]`
...
### ~~FR-V.III-NEW-01: Import DN từ Excel~~ — **BỎ (BA chốt 2026-05-05)**

> **Lý do bỏ:** DN tự đăng ký theo TK-first qua FR-VIII-22 (srs-fr-10). Không cần CB Nghiệp vụ bulk-import DN nữa. DN nào chưa có trong hệ thống → phải tự đăng ký.
...
**Mô tả:** Hiển thị danh sách doanh nghiệp được hỗ trợ pháp lý; hỗ trợ tìm kiếm/lọc đa tiêu chí (từ khóa, quy mô, tỉnh/thành, lĩnh vực, khoảng thời gian hỗ trợ); cho phép Xuất Excel. **KHÔNG có nút "Thêm mới" / "Import Excel"** — DN được tạo qua self-registration FR-VIII-22 ở srs-fr-10.
...
**Loại màn hình:** Form nhập liệu / Chi tiết (4 tab)
...
**Mô tả:** Import hàng loạt doanh nghiệp từ file Excel (.xlsx ≤ 5MB, ≤ 1.000 dòng) qua wizard 3 bước — Bước 1 Upload file (tải mẫu Excel + kéo thả file) → Bước 2 Kiểm tra & Preview (phân loại Hợp lệ / Lỗi / Trùng MST) → Bước 3 Kết quả Import (báo cáo + chi tiết lỗi tải xuống). Chiến lược merge: chỉ INSERT dòng hợp lệ, BỎ QUA dòng trùng MST và dòng lỗi.
**URL:** `/doanh-nghiep/import`
**Quyền truy cập:** Cán bộ nghiệp vụ (TW / Bộ ngành / Địa phương) có quyền CRUD doanh nghiệp. Phạm vi dữ liệu theo BR-AUTH-08; BN/ĐP chỉ import DN thuộc `tinh_thanh_id` của đơn vị mình (validate cột `tinh_thanh_id` từng dòng Excel).
...
| 3 | DON_VI | referenced | Cơ quan/đơn vị (2 tầng: TW → {BN, ĐP} ngang cấp) |
...
**Volume & Growth:** ~10,000 records/năm. Tạo qua self-registration (FR-VIII-22 ở srs-fr-10).
