# SRS — Section 3.2.10: Đánh giá Hiệu quả Hỗ trợ

**Dự án:** Phần mềm hỗ trợ pháp lý doanh nghiệp
**Phiên bản SRS:** 3.0
**Nhóm:** VI — Đánh giá Hiệu quả Hỗ trợ
...
```
[NHAP] →(hoàn tất KH)→ [DA_LAP_KH]
[DA_LAP_KH] →(phân công)→ [CHO_DUYET_PC]
[CHO_DUYET_PC] →(duyệt PC)→ [DA_DUYET_PC]
  →(từ chối PC)→ [DA_LAP_KH]
[DA_DUYET_PC] →(chọn VV)→ [DANG_DANH_GIA]
[DANG_DANH_GIA] →(hoàn tất nhập KQ)→ [DA_DANH_GIA]
[DA_DANH_GIA] →(lập BC)→ [DA_LAP_BC]
[DA_LAP_BC] →(trình duyệt)→ [CHO_DUYET_BC]
[CHO_DUYET_BC] →(duyệt)→ [DA_DUYET_BC]
  →(từ chối)→ [DA_LAP_BC]
```
...
**Priority:** Essential
**Stability:** High
**Màn hình:** SCR-VI-01 — [Kế hoạch Đánh giá](#scr-vi-01-kế-hoạch-đánh-giá)

**Mô tả:**
Tạo đợt đánh giá hiệu quả HTPLDN với thông tin kỳ đánh giá, tần suất, đối tượng. Hệ thống tự sinh mã đợt, gán trạng thái ban đầu NHAP.
...
| 6 | Gán trạng thái ban đầu NHAP | SM-DANHGIA |
...
|---|-----|-----------|-----------|--------|
| 1 | Đợt đánh giá mới | structured | Luôn | Bản ghi DOT_DANH_GIA |
...
- Bản ghi DOT_DANH_GIA được tạo với trạng thái NHAP
...
- **Given** CB NV truy cập "Kế hoạch đánh giá" **When** hiển thị **Then** danh sách KH thuộc đơn vị, phân trang
- **Given** CB NV thêm mới **When** nhập đủ trường **Then** validate + lưu, trạng thái NHAP
...
| # | Tên field | Kiểu logic | Bắt buộc | Ràng buộc | Mặc định | Nguồn |
|---|----------|-----------|----------|-----------|----------|-------|
| 1 | dot_danh_gia_id | identifier | Y | FK → DOT_DANH_GIA | — | Context |
...
- Đợt đánh giá ở trạng thái DA_LAP_KH
...
| 3 | Kiểm tra đợt ở trạng thái DA_LAP_KH | SM-DANHGIA |
...
**Business Rules áp dụng:**
- **SM-DANHGIA**: Transition DA_LAP_KH → CHO_DUYET_PC → Xem Phụ lục C (file chính)
...
| E4 | Đợt không ở DA_LAP_KH | ERR-DG-PC-04 | "Đợt không ở trạng thái phù hợp để phân công" | ERROR |
...
| 5 | Nếu DUYỆT: chuyển trạng thái sang DA_DUYET_PC. Ghi người duyệt, thời gian | SM-DANHGIA |
| 6 | Nếu TỪ CHỐI: chuyển trạng thái sang DA_LAP_KH + ghi lý do. Ghi người từ chối, thời gian | SM-DANHGIA |
| 7 | Gửi thông báo CB NV trình | — |
| 8 | Ghi nhật ký thao tác | BR-DATA-05 |

**Business Rules áp dụng:**
- **SM-DANHGIA**: Transition CHO_DUYET_PC → DA_DUYET_PC (duyệt) / DA_LAP_KH (từ chối) → Xem Phụ lục C (file chính)
...
| 2 | Trạng thái đợt mới | text | Luôn | DA_DUYET_PC / DA_LAP_KH |

**Postconditions (Trạng thái sau thực hiện):**

- Nếu duyệt: đợt → DA_DUYET_PC
- Nếu từ chối: đợt → DA_LAP_KH, CB NV điều chỉnh
- Thông báo gửi CB NV
...
- **Given** CB PD duyệt **When** xác nhận **Then** trạng thái → DA_DUYET_PC
...
- Đợt đánh giá ở trạng thái DA_DUYET_PC
...
| 7 | Chuyển đợt sang trạng thái DANG_DANH_GIA | SM-DANHGIA |
| 8 | Ghi nhật ký thao tác | BR-DATA-05 |

**Business Rules áp dụng:**
- **SM-DANHGIA**: Transition DA_DUYET_PC → DANG_DANH_GIA → Xem Phụ lục C (file chính)
...
| E2 | Đợt không ở DA_DUYET_PC | ERR-DG-VV-01 | "Đợt không ở trạng thái phù hợp" | ERROR |
...
- Đợt ở trạng thái DANG_DANH_GIA
...
| 3 | Kiểm tra đợt ở trạng thái DANG_DANH_GIA | SM-DANHGIA |
...
| 8 | Kiểm tra: tất cả VV trong đợt đã đánh giá? Nếu có → chuyển đợt sang DA_DANH_GIA | SM-DANHGIA |
| 9 | Ghi nhật ký thao tác | BR-DATA-05 |

**Business Rules áp dụng:**
- **SM-DANHGIA**: Transition DANG_DANH_GIA → DA_DANH_GIA (khi tất cả VV hoàn thành) → Xem Phụ lục C (file chính)
...
- Khi tất cả VV hoàn thành: đợt → DA_DANH_GIA
...
| 7 | Chuyển đợt sang trạng thái DA_LAP_BC | SM-DANHGIA |
| 8 | Ghi nhật ký thao tác | BR-DATA-05 |
...
**Business Rules áp dụng:**
- **SM-DANHGIA**: Transition DA_DANH_GIA → DA_LAP_BC → Xem Phụ lục C (file chính)
...
- Đợt chuyển sang DA_LAP_BC

**Error Handling (Xử lý lỗi):**

| # | Điều kiện lỗi | Mã lỗi | Phản hồi hệ thống | Severity |
|---|--------------|--------|-------------------|----------|
| E1 | Đợt không ở DA_DANH_GIA | ERR-DG-BC-01 | "Đợt chưa hoàn thành đánh giá" | ERROR |
...
|------|-------------|-----------|
| 1 | Kiểm tra quyền truy cập | BR-AUTH-01 |
| 2 | Kiểm tra đợt ở trạng thái DA_LAP_BC | SM-DANHGIA |
| 3 | Kiểm tra BC đầy đủ dữ liệu (cảnh báo nếu thiếu) | — |
| 4 | Chuyển trạng thái sang CHO_DUYET_BC | SM-DANHGIA |
| 5 | Gửi thông báo CB PD | — |
| 6 | Ghi nhật ký thao tác | BR-DATA-05 |

**Business Rules áp dụng:**
- **SM-DANHGIA**: Transition DA_LAP_BC → CHO_DUYET_BC → Xem Phụ lục C (file chính)

**Outputs (Dữ liệu đầu ra):**

| # | Tên | Kiểu logic | Điều kiện | Format |
|---|-----|-----------|-----------|--------|
| 1 | Trạng thái mới | text | Luôn | CHO_DUYET_BC |
| 2 | Thông báo CB PD | text | Luôn | Gửi CB PD |

**Postconditions (Trạng thái sau thực hiện):**

- Đợt chuyển sang CHO_DUYET_BC
- Thông báo gửi CB PD
...
| # | Điều kiện lỗi | Mã lỗi | Phản hồi hệ thống | Severity |
|---|--------------|--------|-------------------|----------|
| E1 | Đợt không ở DA_LAP_BC | ERR-DG-TR-01 | "Đợt không ở trạng thái đã lập BC" | ERROR |
| E2 | BC thiếu dữ liệu | WRN-DG-TR-01 | "Báo cáo thiếu thông tin: {danh sách trường}" | WARNING |

**Acceptance Criteria:**

- **Given** CB NV chọn BC hoàn chỉnh **When** nhấn "Trình phê duyệt" **Then** BC → CHO_DUYET_BC, gửi thông báo CB PD
...
| 2 | Kiểm tra đợt ở trạng thái CHO_DUYET_BC | SM-DANHGIA |
| 3 | Hiển thị nội dung BC + số liệu tổng hợp | — |
| 4 | Nếu DUYỆT: chuyển trạng thái sang DA_DUYET_BC. Ghi người duyệt, thời gian | SM-DANHGIA |
| 5 | Nếu TỪ CHỐI: chuyển trạng thái sang DA_LAP_BC + ghi lý do → trả lại CB NV | SM-DANHGIA |
...
**Business Rules áp dụng:**
- **SM-DANHGIA**: Transition CHO_DUYET_BC → DA_DUYET_BC (duyệt) / DA_LAP_BC (từ chối) → Xem Phụ lục C (file chính)
...
|---|-----|-----------|-----------|--------|
| 1 | Kết quả phê duyệt | text | Luôn | DUYET / TU_CHOI |
| 2 | Trạng thái đợt mới | text | Luôn | DA_DUYET_BC / DA_LAP_BC |
| 3 | Thông tin kết quả | structured | Luôn | Trạng thái, người duyệt, thời gian, lý do |

**Postconditions (Trạng thái sau thực hiện):**

- Nếu duyệt: đợt → DA_DUYET_BC
- Nếu từ chối: đợt → DA_LAP_BC, CB NV chỉnh sửa
- Thông báo gửi CB NV
...
| # | Điều kiện lỗi | Mã lỗi | Phản hồi hệ thống | Severity |
|---|--------------|--------|-------------------|----------|
| E1 | Đợt không ở CHO_DUYET_BC | ERR-DG-PD-03 | "Đợt không ở trạng thái chờ duyệt BC" | ERROR |
...
- **Given** CB PD duyệt **When** xác nhận **Then** trạng thái → DA_DUYET_BC
- **Given** CB PD từ chối **When** nhập lý do **Then** → DA_LAP_BC, trả lại CB NV
...
### SCR-VI-01: Đánh giá Hiệu quả HTPL (CONSOLIDATED v2.1)

**Loại màn hình:** Danh sách đợt ĐG + Chi tiết đợt (4 Tabs + Action buttons phê duyệt)
**FR sử dụng:** FR-VI-01, FR-VI-02, FR-VI-03, FR-VI-04, FR-VI-05, FR-VI-06, FR-VI-07, FR-VI-08, FR-VI-09
...
|---|------|-----------|------|---------------------|---------|-------------------|
| 1 | toolbar | Breadcrumb | C01 | "Trang chủ > Đánh giá > Kế hoạch đánh giá" | navigate | Luôn |
| 2 | toolbar | Tiêu đề trang | C02 | "Kế hoạch Đánh giá Hiệu quả" + [+ Tạo đợt đánh giá] [Xuất Excel] [Làm mới] | click → tương ứng | Luôn. [+ Tạo đợt] khi có quyền "Quản lý đánh giá" |
...
| 6 | filter-bar | Lọc trạng thái | C10 dropdown | Tất cả / NHAP / DA_LAP_KH / CHO_DUYET_PC / DA_DUYET_PC / DANG_DANH_GIA / DA_DANH_GIA / DA_LAP_BC / CHO_DUYET_BC / DA_DUYET_BC | change → filter | Luôn |
...
| 15 | table | Trạng thái | C06 badge | Mã màu: 🔵 NHAP, 🟡 CHO_DUYET_PC/CHO_DUYET_BC, 🟢 DA_DUYET_PC/DA_DUYET_BC, 🟠 DANG_DANH_GIA, ⚪ DA_LAP_KH/DA_DANH_GIA/DA_LAP_BC | — | Luôn |
| 16 | table | Người tạo | text | Họ tên CB NV | — | Luôn |
| 17 | table | Ngày tạo | text | dd/mm/yyyy HH:mm | — | Luôn |
| 18 | table | Hành động | icon-group | Xem / Sửa (chỉ NHAP/DA_LAP_KH) / Xóa (chỉ NHAP) | click → tương ứng | Luôn |
...
| 39 | action-bar | Nút [Phê duyệt PC] | C08 Primary | **Quyền: CB PD.** → C12 confirm → SET DA_DUYET_PC. Auto-fill Common Approval Fields | click → approve | Khi TT = CHO_DUYET_PC, vai trò CB PD |
| 40 | action-bar | Nút [Từ chối PC] | C08 Danger | **Quyền: CB PD.** → modal lý do bắt buộc >=10 ký tự → SET DA_LAP_KH | click → reject | Khi TT = CHO_DUYET_PC, vai trò CB PD |
...
| 45 | action-bar | Nút [Hoàn tất chấm điểm] | C08 Primary | SET DA_DANH_GIA. Chỉ khi tất cả VV đã chấm | click → transition | Khi tất cả VV hoàn thành |
...
| 50 | action-bar | Nút [Trình duyệt BC] | C08 Primary | SET CHO_DUYET_BC + gửi TB CB PD | click → transition | Khi TT = DA_DANH_GIA hoặc DA_LAP_BC |
| 51 | action-bar | Nút [Phê duyệt BC] | C08 Primary | **Quyền: CB PD.** SET DA_DUYET_BC | click → approve | Khi TT = CHO_DUYET_BC, vai trò CB PD |
| 52 | action-bar | Nút [Từ chối BC] | C08 Danger | **Quyền: CB PD.** Modal lý do → SET DA_LAP_BC | click → reject | Khi TT = CHO_DUYET_BC, vai trò CB PD |
...
- Tab Thực hiện: lưu nháp cho chấm một phần (is_draft = 1). Khi tất cả VV hoàn thành → auto SET DA_DANH_GIA. 0 VV hoàn thành → Empty State
...
|-------|-----|----------------------------|
| BR-AUTH-01 | Xác thực truy cập | FR-VI-01, 02, 03, 04, 05, 06, 07, 08, 09 |
| BR-AUTH-05 | Phê duyệt cùng cấp | FR-VI-04, 09 |
| BR-DATA-03 | Common fields | FR-VI-01 |
| BR-DATA-04 | Auto-gen mã | FR-VI-01 |
| BR-DATA-05 | Audit trail | FR-VI-01, 02, 03, 04, 05, 06, 07, 08, 09 |
...
| BR-NOTIF-01 | Gửi thông báo kết quả phê duyệt | FR-VI-09 |

### BR-AUTH-01: Xác thực truy cập

Mọi user phải xác thực trước khi truy cập hệ thống.

**Applied in (nhóm VI):** FR-VI-01, FR-VI-02, FR-VI-03, FR-VI-04, FR-VI-05, FR-VI-06, FR-VI-07, FR-VI-08, FR-VI-09
...
**Applied in (nhóm VI):** FR-VI-09

---

**--- Hết file FR Group: VI — Đánh giá Hiệu quả Hỗ trợ ---**
