...
graph LR
    ...
    I --> B
```
> `[GAP-IV-04]` Đồng bộ 9 trạng thái với SM-TVV Section 5 (thêm CHO_THAM_DINH, TU_CHOI → CHO_THAM_DINH, TAM_DUNG → VO_HIEU_HOA)
...
**Mô tả:** Ứng viên TVV/CG (cá nhân ngoài hành nghề tư vấn theo NĐ 77/2008) tự đăng ký tham gia mạng lưới tư vấn viên hỗ trợ pháp lý qua chuyên trang.

**Tác nhân:** Ứng viên TVV/CG (cá nhân ngoài hành nghề tư vấn — chưa có tài khoản hệ thống nội bộ; có thể đăng nhập chuyên trang qua VNeID Tier 2 hoặc đăng ký mới qua chuyên trang công khai)

**Preconditions:** Ứng viên đã xác thực danh tính qua chuyên trang.

**Inputs:**

| # | Tên field | Kiểu logic | Bắt buộc | Ràng buộc | Mặc định | Nguồn |
|---|----------|-----------|----------|-----------|----------|-------|
| 0 | loai_tvv | text | Y | CHECK IN ('TVV','CG') | 'TVV' | user input (radio) |
...
| 8 | so_nam_kinh_nghiem | number | Y | >= 0 | — | user input `[CR-03][Q-03]` |
| 9 | linh_vuc_ids | identifier[] | Y | Lĩnh vực PL đăng ký | — | user input |
| 10 | to_chuc_id | identifier | N | FK → TO_CHUC_TU_VAN | — | user input |
| 11 | file_bang_cap | binary[] | Y | PDF, max 10MB/file | — | user upload |
| 12 | file_the_hanh_nghe | binary | N | PDF, max 10MB (bắt buộc nếu loai_tvv='TVV' theo NĐ 77/2008 Đ.20) | — | user upload |
...
| 5 | Virus scan ClamAV cho tất cả file upload (timeout 30s) | — |
| 6 | Tạo bản ghi TU_VAN_VIEN với `loai_tvv` theo input ('TVV' hoặc 'CG'), trạng thái = MOI_DANG_KY | SM-TVV |
| 7 | Gửi thông báo cho CB NV (NHT cấp ĐP/BN/TW theo lĩnh vực đăng ký) | — |
...
| E6 | Virus scan phát hiện | ERR-DK-06 | "File {ten_file} chứa mã độc, bị từ chối" | ERROR |
...
**Postconditions:**
- Hồ sơ TVV/CG được tạo với trạng thái MOI_DANG_KY
- CB NV (NHT) đơn vị nhận thông báo hồ sơ mới
- Ứng viên theo dõi được trạng thái qua chuyên trang

**Acceptance Criteria:**
- **Given** ứng viên TVV/CG truy cập chuyên trang **When** chọn "Đăng ký tham gia mạng lưới TVV" **Then** form đăng ký
- **Given** ứng viên nhập đủ + upload file **When** gửi **Then** tạo hồ sơ, trạng thái = MOI_DANG_KY, CB NV (NHT) nhận thông báo
- **Given** ứng viên đã có hồ sơ chờ **When** đăng ký lại **Then** hệ thống từ chối
...
**Mô tả:** TVV/CG (cá nhân ngoài, chủ hồ sơ) tự cập nhật thông tin năng lực qua chuyên trang; HOẶC CB NV (NHT — cán bộ HTPL theo NĐ 55/2019 Đ.7) cập nhật thay khi được phân công.

**Tác nhân:** TVV/CG (chủ hồ sơ) qua chuyên trang / CB NV (NHT)

**Preconditions:** TVV tồn tại; TVV/CG là chủ hồ sơ HOẶC CB NV (NHT) có quyền theo phân công VAI_TRO + đơn vị.
...
**Acceptance Criteria:**
- **Given** TVV/CG xem hồ sơ của mình qua chuyên trang **When** nhấn "Cập nhật năng lực" **Then** form inline edit
- **Given** TVV/CG cập nhật thông tin/chứng chỉ + upload file **When** lưu **Then** validate và lưu thành công
- **Given** CB NV (NHT) xem chi tiết TVV **When** cập nhật năng lực **Then** validate + lưu (theo phân công VAI_TRO)
...
| 2 | Nếu PHE_DUYET: chuyển trạng thái HOAT_DONG, set `ngay_cong_nhan = NOW()`, `thoi_gian_duyet = NOW()`, `nguoi_duyet = current_user.id`, `so_quyet_dinh`, tăng `version` | SM-TVV |
| 3 | Nếu TU_CHOI: chuyển trạng thái TU_CHOI, set `thoi_gian_tu_choi`, `nguoi_tu_choi`, `ly_do_tu_choi`, tăng `version` | SM-TVV |
| 4 | Gửi thông báo TVV/CG (chủ hồ sơ) | — |
...
- TVV/CG (chủ hồ sơ) nhận thông báo kết quả
...
- **Given** CB PD phê duyệt **When** xác nhận **Then** TVV → HOAT_DONG, ghi audit log, TVV/CG (chủ hồ sơ) nhận thông báo
...
| Bước | Mô tả xử lý | BR áp dụng |
|------|-------------|-----------|
| 1 | Kiểm tra đối tượng (TVV hoặc TC TV) ở trạng thái HOAT_DONG | SM-TVV `[CR-02]` |
| 2 | Công khai: đặt cong_khai = 1, auto fill thoi_gian_dang_tai, gọi API Cổng PLQG | BR-PUBLIC-01 |
| 3 | Hủy công khai: đặt cong_khai = 0, clear thoi_gian_dang_tai, gọi API gỡ khỏi Cổng | BR-PUBLIC-02 |
...
| # | Điều kiện lỗi | Mã lỗi | Phản hồi hệ thống | Severity |
|---|--------------|--------|-------------------|----------|
| E1 | TVV không HOAT_DONG | ERR-CK-01 | "Chỉ tư vấn viên đang hoạt động mới được công khai" | ERROR |
| E2 | API Cổng PLQG lỗi | WRN-CK-01 | "Cập nhật Cổng pháp luật quốc gia thất bại, sẽ thử lại" | WARNING |
...
### FR-IV-11: TVV/CG cập nhật hồ sơ cá nhân (UC49)

**UC Reference:** UC 49
**Priority:** Essential | **Stability:** High
**Màn hình:** SCR-IV-02 (chuyên trang)

**Mô tả:** TVV/CG (cá nhân ngoài, chủ hồ sơ) tự cập nhật thông tin hồ sơ cá nhân của mình qua chuyên trang.

**Tác nhân:** TVV/CG (chủ hồ sơ — đăng nhập chuyên trang qua VNeID Tier 2)
...
| Bước | Mô tả xử lý | BR áp dụng |
|------|-------------|-----------|
| 1 | Kiểm tra TVV/CG sở hữu hồ sơ (`current_user.cccd = TU_VAN_VIEN.cccd`) | BR-AUTH-01 |
| 2 | **Kiểm tra trạng thái TVV trước save**: nếu `trang_thai = VO_HIEU_HOA` → reject ERR-CN-03 (chống race condition khi CB NV vô hiệu hóa song song với TVV/CG sửa) | — |
| 3 | Xác nhận dữ liệu đầu vào (email format, SĐT format) | — |
| 4 | Cập nhật TU_VAN_VIEN | — |
...
| E3 | TVV đã VO_HIEU_HOA | ERR-CN-03 | "Hồ sơ đã bị vô hiệu hóa, không thể chỉnh sửa. Vui lòng liên hệ cán bộ quản lý" | ERROR |

**Acceptance Criteria:**
- **Given** TVV/CG truy cập chuyên trang "Thông tin cá nhân" **When** chọn chỉnh sửa **Then** hiển thị form cập nhật
- **Given** TVV/CG thay đổi thông tin **When** lưu **Then** validate + cập nhật thành công, ghi audit log
- **Given** TVV/CG lưu thành công **When** hệ thống xử lý xong **Then** hiển thị thông tin đã cập nhật ở chế độ readonly để xác nhận
...
**Mô tả:** Quản trị hệ thống / Cán bộ Nghiệp vụ tạo/sửa/xóa hồ sơ Người hỗ trợ pháp lý (cán bộ HTPL theo NĐ 55/2019 Đ.7) thuộc đơn vị mình. Bao gồm cấp tài khoản đăng nhập (qua TAI_KHOAN) và gán lĩnh vực chuyên môn (cho UC59 phân công vụ việc).
...
- Tài khoản TAI_KHOAN đã tồn tại (hoặc tạo mới đồng thời với NHT)

**Inputs:**

| # | Tên trường | Kiểu | Bắt buộc | Ràng buộc | Mặc định |
|---|------------|------|----------|-----------|----------|
| 1 | tai_khoan_id | định danh | Bắt buộc | UNIQUE 1:1 với NGUOI_HO_TRO; nếu chưa có thì tạo mới | — |
| 2 | don_vi_id | định danh | Bắt buộc | FK → DON_VI; CB Nghiệp vụ chỉ được chọn đơn vị của mình | Đơn vị của user tạo |
| 3 | chuc_vu | văn bản | Không | Tối đa 200 ký tự | — |
| 4 | linh_vuc_ids | danh sách định danh | Bắt buộc | FK → DANH_MUC, ≥ 1 | — |
| 5 | chung_chi_htpl | văn bản dài | Không | JSON gồm: tên khóa bồi dưỡng, ngày bồi dưỡng, nơi bồi dưỡng | — |
| 6 | ngay_tham_gia_mang_luoi | ngày | Không | ≤ ngày hiện tại | — |
| 7 | ghi_chu | văn bản dài | Không | Tối đa 5000 ký tự | — |
...
| 2 | Nếu tạo mới TAI_KHOAN: kiểm tra username/email duy nhất | — |
| 3 | Tạo bản ghi NGUOI_HO_TRO + junction NGUOI_HO_TRO_LINH_VUC | Common fields |
| 4 | Đặt trạng thái = HOAT_DONG | SM-NHT đơn giản |
| 5 | Gán vai trò "NHT" vào TAI_KHOAN_VAI_TRO | — |
| 6 | Gửi email kích hoạt tài khoản cho NHT (nếu tạo mới TAI_KHOAN) | — |
| 7 | Ghi nhật ký thao tác | Nhật ký kiểm toán |
...
| 4 | trang_thai | văn bản | HOAT_DONG / TAM_DUNG / VO_HIEU_HOA |

**Postconditions:** Bản ghi NHT được tạo/cập nhật/xóa mềm. NHT có thể đăng nhập hệ thống (nếu vừa tạo TK).
...
**Acceptance Criteria:**
- **Given** Quản trị viên / CB Nghiệp vụ truy cập "Quản lý NHT" **When** thêm NHT mới **Then** tạo bản ghi NGUOI_HO_TRO + cấp TK + gán vai trò
...
**SM-TVV (Tư vấn viên / Chuyên gia — 9 trạng thái):**
...
**SM-NHT (Người hỗ trợ pháp lý — 3 trạng thái):**
...
| MD-CONG-KHAI | Xác nhận công khai lên Cổng pháp luật quốc gia? | Sau khi công khai, thông tin **{tên}** sẽ hiển thị toàn quốc trên Cổng pháp luật quốc gia. Bạn có thể hủy công khai bất kỳ lúc nào. | Công khai |
...
| 14 | bộ lọc | Trạng thái | dropdown chọn nhiều | 9 trạng thái theo bảng ánh xạ § 3.0; ẩn khi đang ở tab cụ thể | Lọc theo trạng thái |
...
**FR sử dụng:** FR-IV-01 (CRUD), FR-IV-03 (đăng ký), FR-IV-04 (cập nhật năng lực)
**Đường dẫn:** `/chuyen-gia-tvv/tao-moi` hoặc `/chuyen-gia-tvv/:id/chinh-sua`
**Quyền truy cập:**
- Cán bộ Nghiệp vụ: tạo/sửa tư vấn viên thuộc đơn vị mình
- Ứng viên Tư vấn viên / Chuyên gia: tự đăng ký qua chuyên trang Cổng pháp luật quốc gia
- Tư vấn viên / Chuyên gia (chủ hồ sơ): cập nhật hồ sơ của mình qua chuyên trang
- Chỉ cho phép sửa khi trạng thái khác "Vô hiệu hóa"

**Mô tả:** Biểu mẫu chia 5 nhóm thu gọn được: Thông tin cá nhân, Thông tin nghề nghiệp, Tổ chức & Mạng lưới, File đính kèm, Ghi chú. Khi tạo mới → đặt trạng thái "Mới đăng ký" và gửi thông báo Cán bộ Nghiệp vụ cùng đơn vị.
...
| 10 | header | Nút **Công khai lên Cổng pháp luật quốc gia** | nút chính | "Công khai" (tooltip: "Đẩy thông tin lên Cổng pháp luật quốc gia") | Click → MD-CONG-KHAI → gọi API Cổng pháp luật quốc gia → đặt cong_khai = 1, ghi thời gian đăng tải | Vai trò = Cán bộ Nghiệp vụ có quyền công khai; trạng thái = Đang hoạt động AND chưa công khai |
...
| # | Vùng | Thành phần | Loại UI | Label / Dữ liệu hiển thị | Hành vi | Điều kiện hiển thị |
|---|------|-----------|---------|---------------------------|---------|---------------------|
| 12 | tab 1 | Tab "Hồ sơ" | tab + nội dung | 5 nhóm thu gọn được, chỉ đọc: (a) Thông tin cá nhân (loại + họ tên + ngày sinh + giới tính + Căn cước công dân + email + số điện thoại + địa chỉ); (b) Nghề nghiệp (trình độ, chứng chỉ, số thẻ, kinh nghiệm); (c) Tổ chức (tổ chức chính link hoặc "Tự do", đối tác dạng thẻ); (d) Lĩnh vực (thẻ); (e) File đính kèm (tên + kích thước + nút "Xem" mở hộp xem PDF, "Tải xuống") | — | Luôn (chủ hồ sơ chỉ thấy hồ sơ của mình) |
...
**Loại màn hình:** Biểu mẫu nhập liệu (3 nhóm)
...
**Mô tả:** Biểu mẫu chia 3 nhóm: Tài khoản & Đơn vị, Chuyên môn, Bồi dưỡng. Khi tạo mới có thể tạo TAI_KHOAN đăng nhập đồng thời + gửi email kích hoạt.
...
| 2.1 | nhóm 1 | Chế độ tạo tài khoản | radio | "Tạo tài khoản mới" / "Liên kết tài khoản đã có" — chỉ hiện khi tạo mới | Đổi chế độ → hiển thị field tương ứng |
| 2.2 | nhóm 1 | Họ tên * | ô văn bản | Hiển thị khi tạo mới (chế độ tạo TK mới) HOẶC sửa | — |
| 2.3 | nhóm 1 | Email * | ô văn bản | Đúng định dạng email; duy nhất (chế độ tạo TK mới) | Lỗi trùng: "Email đã được sử dụng" |
| 2.4 | nhóm 1 | Tên đăng nhập * | ô văn bản | Duy nhất (chế độ tạo TK mới) | Lỗi trùng: "Tên đăng nhập đã được sử dụng" |
| 2.5 | nhóm 1 | Tài khoản đã có | dropdown có tìm kiếm | Hiển thị khi chế độ "Liên kết tài khoản đã có" — chọn tài khoản chưa được liên kết với Người hỗ trợ pháp lý | — |
| 2.6 | nhóm 1 | Đơn vị công tác * | dropdown có tìm kiếm | Sở Tư pháp / Bộ ngành / UBND / tổ chức đại diện DN; Cán bộ Nghiệp vụ chỉ chọn được đơn vị của mình; mặc định = đơn vị của user tạo | Lỗi vượt quyền: "Bạn không có quyền tạo Người hỗ trợ pháp lý cho đơn vị này" |
| 2.7 | nhóm 1 | Chức vụ | ô văn bản | Tùy chọn, tối đa 200 ký tự (VD: Chuyên viên, Trưởng phòng) | — |
...
| 3.2 | nhóm 2 | Ngày tham gia mạng lưới | bộ chọn ngày | Tùy chọn, ≤ hôm nay | — |
| 4 | nhóm 3 | Bồi dưỡng | nhóm thu gọn | — | — |
| 4.1 | nhóm 3 | Lịch sử bồi dưỡng | bảng (lặp dòng) | Mỗi dòng: Tên khóa bồi dưỡng + Ngày bồi dưỡng + Nơi bồi dưỡng + Số chứng chỉ (nếu có); căn cứ NĐ 55/2019 Đ.7 | + Thêm dòng / Xóa dòng |
| 5 | nhóm 4 | Ghi chú | ô văn bản dài | Tối đa 5000 ký tự, không bắt buộc | — |
| 6 | thanh hành động | 2 nút Hủy / Lưu | nhóm nút | "Hủy" (phụ) / "Lưu" (chính) | Hủy: nếu có thay đổi chưa lưu → MD-XOA xác nhận. Lưu: tạo mới hoặc cập nhật + (nếu chế độ tạo TK mới) gửi email kích hoạt + gán vai trò "NHT" cho tài khoản |

#### Quy tắc tương tác
- **Khi tạo mới + chế độ "Tạo tài khoản mới":** hệ thống tạo TAI_KHOAN với trạng thái "Chờ kích hoạt" + gửi email kích hoạt cho NHT (link đặt mật khẩu).
- **Khi liên kết tài khoản đã có:** chỉ chấp nhận TAI_KHOAN chưa liên kết với NHT khác (do quan hệ 1:1).
...
**Loại màn hình:** Trang chi tiết 3 tab + 2 nút hành động ở header
...
**Mô tả:** Hồ sơ chi tiết NHT với 3 tab: Thông tin (tài khoản + đơn vị + chuyên môn), Bồi dưỡng (lịch sử các khóa bồi dưỡng theo NĐ 55/2019 Đ.7), Vụ việc đã hỗ trợ (lịch sử vụ việc UC60/UC65 mà Người hỗ trợ pháp lý đã được phân công).
...
| # | Vùng | Thành phần | Loại UI | Label / Dữ liệu hiển thị | Hành vi |
|---|------|-----------|---------|---------------------------|---------|
| 6 | tab 1 | Tab "Thông tin" | tab + nội dung | 2 nhóm thu gọn được, chỉ đọc: (a) Tài khoản & Đơn vị (Họ tên + Email + Tên đăng nhập + Đơn vị công tác + Chức vụ + Ngày tham gia mạng lưới); (b) Chuyên môn (lĩnh vực dạng thẻ, ghi chú) | — |
| 7 | tab 2 | Tab "Bồi dưỡng" | tab + nội dung | Bảng lịch sử bồi dưỡng theo NĐ 55/2019 Đ.7: Số thứ tự + Tên khóa bồi dưỡng + Ngày bồi dưỡng + Nơi bồi dưỡng + Số chứng chỉ (link tải file nếu có) | — |
| 7b | tab 2 | Trạng thái rỗng | minh họa + label | Khi chưa có khóa bồi dưỡng: "Chưa có lịch sử bồi dưỡng. Vui lòng cập nhật qua menu 'Sửa hồ sơ'" | — |
| 8 | tab 3 | Tab "Vụ việc đã hỗ trợ" | tab + nội dung | (a) Bộ lọc: Khoảng ngày + Trạng thái vụ việc (chọn nhiều: "Tất cả" / "Đang xử lý" / "Hoàn thành" / "Đã hủy"); (b) Bảng: Mã vụ việc (đường liên kết) + Tên vụ việc + Doanh nghiệp + Lĩnh vực + Ngày phân công + Ngày hoàn thành + Kết quả; (c) Thống kê tóm tắt: "Tổng vụ việc: {N}", "Đang xử lý: {M}", "Hoàn thành: {K}" | Click mã vụ việc → SCR-V-* chi tiết vụ việc |
| 8b | tab 3 | Phân trang | pagination | 20 vụ việc/trang | — |
| 8c | tab 3 | Trạng thái rỗng | minh họa + label | Khi chưa có vụ việc: "Người hỗ trợ pháp lý chưa được phân công vụ việc nào" | — |
...
| trang_thai | text | Y | CHECK IN ('MOI_DANG_KY','CHO_THAM_DINH','DANG_THAM_DINH','YEU_CAU_BO_SUNG','CHO_PHE_DUYET','TU_CHOI','HOAT_DONG','TAM_DUNG','VO_HIEU_HOA') | 'MOI_DANG_KY' | Trạng thái lifecycle (SM-TVV) |
...
| 8 | trang_thai | text | Y | CHECK IN ('HOAT_DONG','TAM_DUNG','VO_HIEU_HOA') | 'HOAT_DONG' | Trạng thái — đơn giản 3 trạng thái (cán bộ không cần workflow thẩm định 4 tiêu chí như TVV cá nhân) |
...
| Workflow | MOI_DANG_KY → CHO_THAM_DINH → DANG_THAM_DINH → CHO_PHE_DUYET → HOAT_DONG (9 trạng thái) | HOAT_DONG → TAM_DUNG → VO_HIEU_HOA (3 trạng thái đơn giản) |
...
```mermaid
stateDiagram-v2
    [*] --> MOI_DANG_KY : Ứng viên TVV/CG đăng ký tham gia MLTV
    ...
    YEU_CAU_BO_SUNG --> DANG_THAM_DINH : TVV/CG (chủ hồ sơ) bổ sung xong
    DANG_THAM_DINH --> CHO_PHE_DUYET : Thẩm định đạt (4 nhóm tiêu chí)
    DANG_THAM_DINH --> TU_CHOI : Cán bộ Nghiệp vụ kết luận KHÔNG ĐẠT
    CHO_PHE_DUYET --> HOAT_DONG : Cán bộ Phê duyệt phê duyệt (công nhận)
...
|-----------|-----|-------|-------------|
| Mới đăng ký | MOI_DANG_KY | Ứng viên TVV/CG vừa đăng ký tham gia MLTV | Xanh dương |
...
| Yêu cầu bổ sung | YEU_CAU_BO_SUNG | Hồ sơ thiếu, chờ TVV/CG (chủ hồ sơ) bổ sung | Cam |
...
| Đang hoạt động | HOAT_DONG | TVV đã được công nhận, đang hoạt động | Xanh lá |
...
| CHO_PHE_DUYET | HOAT_DONG | Cán bộ Phê duyệt duyệt | Cùng cấp (BR-AUTH-05), có `so_quyet_dinh` | Audit, ngay_cong_nhan, thoi_gian_duyet, nguoi_duyet | FR-IV-07 | BR-AUTH-05 |
