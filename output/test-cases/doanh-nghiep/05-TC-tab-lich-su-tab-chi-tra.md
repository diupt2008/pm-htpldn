...
| TC-HSCT-UI-01 | FR-V.III-01 / SCR-V.III-02 row#4 / UI | Verify trường thông tin Tab #4 "Hồ sơ Chi trả": table columns, read-only constraints, nguồn DVC | 1. cb_nv_tw_01 login. 2. DN-X có ≥1 HSCT. | — | 1. Mở chi tiết DN-X. 2. Click Tab #4. 3. Kiểm tra table: tên cột, thứ tự. 4. Xác nhận KHÔNG có action CRUD. 5. Xác nhận không có KPI section. | (3) **Table HSCT cột**: Mã HSCT (text click→detail cross-module), Mã VV liên kết (text), Số tiền đề nghị (number VND), Số tiền duyệt (number VND), Trạng thái badge (CHO_PHE_DUYET/DA_DUYET/TU_CHOI/DA_THANH_TOAN/HUY), Ngày tạo (date). **Pagination**: 20 mục/trang. **Phần tử KHÔNG có**: không có nút [+ Thêm HSCT] (nguồn duy nhất: DVC qua LGSP — cite NotebookLM [7]), không có nút [Sửa]/[Xóa], không có KPI section (⚠️ SRS row#4 chỉ quote list). | Happy 🔴 |
