...
| 2 | CHUONG_TRINH_HT | `CHUONG_TRINH_HT` (đúng SRS) | ≥3 | **2 pre-existing** | ⚠️ | URL đúng `/CHUONG_TRINH_HT` có 2 record (`CT_NGUOI_NGHEO`/`CT_DTTS`); FE sub-tab routing sai gửi `/CHUONG_TRINH_HO_TRO` BE 422. Form thiếu 3 trường SRS row 3-5. 2-source verify: ✅ NotebookLM + SRS local match. **Bug:** [bug-report-r7-1-6-dm-cthttp.md](../bug-reports/bug-report-r7-1-6-dm-cthttp.md). |
