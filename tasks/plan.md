...
**Ngày viết:** 2026-04-25 · **Phiên bản:** v2.6.8 (2026-05-06 lần 6 — apply SRS update **FR-02 v3.5** đầy đủ stack 12 file QA. **FR-02 v3.5 sync 12 file:** [7.2-hoi-dap-phap-ly.md](../output/funtion/7.2-hoi-dap-phap-ly.md) **35→60 TC** (P0:27 P1:26 P2:4 — 57 active + 3 deprecated) header 13 FR + 25 TC mới HD-040..064 chia 8 nhóm v3.5 + fix 13 UC ref `[fix-v3.5]` (mapping UC v3.5 đổi: UC15 cũ phê duyệt → v3.5 phân công, UC17 cũ đóng → v3.5 gộp phê duyệt/công khai/hủy CK/đóng) + expand 6 TC `[expand-v3.5]` (HD-019/021/027/029/030/031/034) + deprecate 3 TC duplicate (HD-017→HD-056/057, HD-018→HD-058, HD-033→HD-040..044); [6.2-sm-hoidap.md](../output/smoke/6.2-sm-hoidap.md) **7→11 test paths** + 4 path mới (TP-HD-08 phân công Tổ chức TC TV + TP-HD-09 TVN_BRIDGE inbound từ FR-13 + TP-HD-10 đóng hồ sơ thủ công BR-FLOW-06 + TP-HD-11 vai trò Công khai BR-FLOW-05 thu hẹp); [6.2-smoke-hoidap.md](../output/smoke-specs/6.2-smoke-hoidap.md) Bước 2c v3.5 verify 6 mục (7 tabs gộp + cột Cơ quan tiếp nhận + filter 5 kênh có TVN_BRIDGE + dropdown chèn mẫu 2 nhóm Mô hình B + button Đóng HS + form Thêm mới ẩn TVN_BRIDGE) + đổi tên menu "pháp lý" → "pháp luật" + SRS ref đổi sang `srs-update-2026-5-5/srs-fr-02-hoi-dap.md`; 3 [permission-matrix*.md](../output/permission-matrix.md) header bullet FR-02 v3.5 (4 mã quyền action-level mới `MPH_CREATE_TW/BN/DP` + `MPH_READ` + ngoại lệ BR-AUTH-08 cho MAU_PHAN_HOI Mô hình B) + by-fr §FR-02 7 bullet note (đổi tên + tag CHANGED v3.5 cho MAU_PHAN_HOI + 5 trường công khai + CR-06 + TVN_BRIDGE + state HUY + BR-FLOW-05/06) + by-role 11 heading + 11 row description + overview row tổng quan; [funtion/README.md](../output/funtion/README.md) (TC count 36→57 active) + [smoke/README.md](../output/smoke/README.md) (paths 7→11, total 40→44) + [smoke-specs/README.md](../output/smoke-specs/README.md) (menu label); [test-strategy.md](../output/test-strategy.md) **v3.3** (4→5 SRS update + module name + count 27/26/4/57 + SM 11 paths); [qa-tracking.md](../output/qa-tracking.md) (5 instances tên module); [srs-conflicts-need-ba.md](../output/srs-conflicts-need-ba.md) **LỖI A `DA_PHAN_CONG` typo SRS — sao chép nhầm template từ FR-V.I-09 VU_VIEC** (deep review 2026-05-07 cross-check 5 nguồn xác định: Master `srs-v3.md` line 1367 + line 4985-5011 SM-VUVIEC + 02-thu-tu-module.md line 421-487 đã cảnh báo "bám Master" + flow-module.md line 184 SM-VUVIEC 12 state + srs-fr-02 line 1341 CHECK + line 1488 SM-HOIDAP đều canonical = HOI_DAP 9 state KHÔNG có DA_PHAN_CONG. Test plan FR-02 hiện tại đã đúng theo truth → **không block**. BA fix sau: xóa DA_PHAN_CONG khỏi 7 vị trí typo line 317/385/400/404/448/474/498/502/509/511); [tasks/todo.md](todo.md) banner FR-02 v3.5 + row "Hỏi đáp" tag ✏️ + 2 sub-task mới R7.3.1.MoB/R7.3.1.TVN (🟢 sẵn sàng seed-first) + R7.3.1 ✅ giữ nguyên (R6 PASS 6/6 — schema cũ đủ cover entry MOI; v3.5 không add field ở MOI) + R7.4.A4 spec note 11 paths + R7.7.1 spec note 60 TC. **17 thay đổi áp:** đổi tên module "pháp lý" → "pháp luật" Thay đổi 2 FR-11 ITEM-14 (T1) · 5 trường công khai chuẩn CR-01 cho HOI_DAP+PHAN_HOI (`cong_khai`/`anh_dai_dien`/`thoi_gian_dang_tai`/`mo_ta_cong_khai`/`file_dinh_kem_cong_khai`) (T2) · `don_vi_id` CR-06 DN chọn cơ quan tiếp nhận từ Cổng PLQG default Sở TP tỉnh DN (T3) · enum kênh thêm `TVN_BRIDGE` auto-fill từ FR-13 ESCALATE + `tu_van_nhanh_goc_id` link phiên gốc (T4) · state `HUY` mới SM 8→9 state (T5) · field `loai_doi_tuong_xu_ly` CA_NHAN/TO_CHUC + `to_chuc_tu_van_id` validate TVV thuộc TC (T6) · MAU_PHAN_HOI Mô hình B Hybrid 2 tầng `pham_vi_ap_dung` ∈ TW_QUOC_GIA/BN_RIENG/DP_RIENG auto-fill theo cấp user không cho sửa (T7) · 4 mã quyền action-level MPH_CREATE_TW/BN/DP + MPH_READ ngoại lệ BR-AUTH-08 (T8) · BR-FLOW-03 mở rộng cấm sửa/xóa cả CONG_KHAI+HOAN_THANH (T9) · BR-FLOW-05 thu hẹp chỉ CB PD cùng cấp được Công khai/Hủy CK CB NV bị chặn (T10) · BR-FLOW-06 mới Đóng hồ sơ thủ công không auto-close (T11) · SCR-II-01 gộp 7 tabs từ v3 9 tabs (T12) · modal Công khai mới với ảnh đại diện 5MB + mô tả 2000 ký tự + file 20MB×10 (T13) · idempotency key cho API Cổng PLQG (T14) · 2 SCR mới chuyển sang QTHT MH-10.7 (FR-II-NEW-01 Cấu hình phân công + FR-II-NEW-02 Mẫu phản hồi) (T15) · ERR codes mới (ERR-MPH-01..06 Mẫu + ERR-PC-04..06 Tổ chức + ERR-TH-CONFLICT version optimistic locking) (T16) · 7 vai trò chuẩn CB_NV/CB_PD TW/BN/DP + QTHT đồng bộ srs-v3.md §3.4.2 (T17). **✅ BA pending — KHÔNG block test (cosmetic typo SRS):** state `DA_PHAN_CONG` ở FR-II-04/05/06 (7 vị trí line 317/385/400/404/448/474/498/502/509/511) là **typo cherry-pick template từ FR-V.I-09 VU_VIEC sang HOI_DAP**. Cross-check 5 nguồn xác định canonical truth: HOI_DAP 9 state KHÔNG có DA_PHAN_CONG (Master `srs-v3.md` line 1367 dùng cho VU_VIEC; line 4985-5011 SM-VUVIEC; 02-thu-tu-module.md line 487 đã spot từ trước "bám Master"; flow-module.md line 184; srs-fr-02 line 1341 CHECK + line 1488 SM-HOIDAP). Test plan hiện tại đã giả định bỏ → ĐÚNG. BA fix sau cosmetic, không block test plan (HD-030/049/050 mô tả TIEP_NHAN→DANG_XU_LY trực tiếp + SM 9 state + 7 tabs SCR-II-01 đều đúng truth). **FR-II-NEW-02 đã CĐT chốt 2026-05-02** (line 895 SRS — Mô hình B Hybrid 2 tầng, Stability High, full spec) — không phải feature pending. **Phụ thuộc BE deploy (UI-first convention — KHÔNG block trước):** schema migration field v3.5 (5 trường công khai HOI_DAP+PHAN_HOI + `don_vi_id` + `tu_van_nhanh_goc_id` + `loai_doi_tuong_xu_ly` + `to_chuc_tu_van_id` + bảng MAU_PHAN_HOI cột `pham_vi_ap_dung` + enum kênh TVN_BRIDGE + state HUY + 4 mã quyền action-level + endpoint Cổng PLQG idempotency). QA tester walk UI workflow trước (R7.3.1.MoB qua Tab Mẫu phản hồi đã deploy verified bug R7.1.1; R7.3.1.TVN walk FR-13 ESCALATE qua UI sau R7.6.2). Nếu UI submit fail vì BE schema chưa migrate → log dev bug + skip, tiếp tục task khác. Fallback API exception chỉ khi UI sub-menu chưa deploy (giống R7.2.2 TC TV pattern). **Defer:** entity column legacy heuristic chưa update permission-matrix-by-role detail row level (giữ ✅ CRU\*D như v3, scope đổi document ở header bullet). 5 R7 task FR-02 được update (R7.3.1 ✅ giữ nguyên + R7.3.1.MoB 🆕 🟢 + R7.3.1.TVN 🆕 🟢 + R7.4.A4 spec note + R7.7.1 spec note) — xem [todo.md](todo.md) Phase 3 + Phase 4 + Phase 7. Cite: [`srs-update-2026-5-5/srs-fr-02-hoi-dap.md`](../input/srs-update-2026-5-5/srs-fr-02-hoi-dap.md) v3.5 (1.652 dòng) + [`CHANGELOG-v3-to-v3.5.md`](../input/srs-update-2026-5-5/CHANGELOG-v3-to-v3.5.md) cross-ref FR-02 từ FR-04 Thay đổi 18 / FR-10 Thay đổi 13 / FR-11 Thay đổi 2 / FR-16 Thay đổi 1+4.) · v2.6.7 (2026-05-06 lần 5 — apply SRS update **FR-06 v3.5** đầy đủ stack 10 file QA. **FR-06 v3.5 sync 10 file:** [7.6-chi-tra-chi-phi.md](../output/funtion/7.6-chi-tra-chi-phi.md) **30→35 TC** (P0:15 P1:16 P2:4) header 13→14 FR + 5 TC mới CT-032/033/034/035 + sửa CT-006/CT-014; [6.6-sm-chitra.md](../output/smoke/6.6-sm-chitra.md) 9→11 paths + Mermaid v3.5 + 14 transition; [6.6-smoke-chitra.md](../output/smoke-specs/6.6-smoke-chitra.md) account `cb_nv_tw_01` + URL `/chi-tra/danh-sach` + negative test "không nút Thêm mới" + UI tiếng Việt thuần verify; [test-strategy.md §7.6](../output/test-strategy.md) 13→14 FR; [entity-map.md](../input/data/entity-map.md) update E09 (+9 fields lifecycle + UNIQUE `ma_ho_so_dvc`) + thêm E34 `THAM_DINH_HO_SO` (1:1) + E35 `PHE_DUYET_CHI_TRA` (N:1); [flow-module.md §10](../input/quy-trinh-nghiep-vu/flow-module.md) 7 bước → 12 bước (thêm 5 sub-flow: rút HS / bổ sung qua DVC / từ chối kiểm tra / từ chối thẩm định / CB PD trả về / từ chối thanh toán); [02-thu-tu-module.md §⑪](../input/quy-trinh-nghiep-vu/02-thu-tu-module.md) refactor 8 section SCR-V.II-02 + bảng 14 transition + bỏ row auto-từ-chối Thay đổi 5 OUT; 3 [permission-matrix*.md](../output/permission-matrix.md) +THAM_DINH_HO_SO + PHE_DUYET_CHI_TRA × 7 role (QTHT R + CB_NV TW/BN/ĐP CRU* on THAM_DINH + R on PHE_DUYET + CB_PD TW/BN/ĐP R* on THAM_DINH + CR* on PHE_DUYET BR-AUTH-05) + DN 🔌 C†R*→🔌 C†RU* (FR-V.II-14 update qua DVC); [tasks/todo.md](todo.md) banner FR-06 v3.5 + R7.6.1/R7.7.12 update + 4 subtask mới R7.7.12.1/2/3/4 + 5 defer items Open issues. **9 thay đổi áp:** đồng bộ 10 trạng thái SM-CHITRA bỏ `MOI`/`DA_TIEP_NHAN`/`CHO_THAM_DINH`/`DA_THAM_DINH`/`TU_CHOI_THAM_DINH`/`TU_CHOI_THANH_TOAN` (T1) · CB PD "Từ chối" = trả về `DANG_THAM_DINH` lý do ≥10 ký tự BR-FLOW-04 KHÔNG phải Từ chối cuối (T2) · 2 sub-flow FR-V.II-02 Tiếp nhận `[GAP-V.II-02]` + DN rút `[GAP-V.II-03]` (T3) · FR-V.II-14 mới DN bổ sung qua DVC/Cổng PLQG/CB NV thủ công max 3 lần ≤5 ngày LV (T4) · 2 entity owned mới `THAM_DINH_HO_SO` (1:1, ~3.000/năm) + `PHE_DUYET_CHI_TRA` (N:1 cho phép CB PD trả về nhiều lần, ~3.500/năm) + reference THONG_BAO (T6) · HO_SO_CHI_TRA +9 fields lifecycle (`ngay_tiep_nhan`/`nguoi_tiep_nhan_id`/`thoi_gian_tu_choi`/`nguoi_tu_choi_id`/`ly_do_huy`/`bo_sung_count` CHECK 0-3/`ngay_yeu_cau_bo_sung`) + UNIQUE `ma_ho_so_dvc` idempotent ERR-CT-02 (T7 cascading) · UI tiếng Việt thuần SCR-V.II-01/02 bỏ raw enum (T9) · Mô tả/URL/Quyền cho SCR-V.II-01/02 (T10) · DON_VI 2 tầng đồng bộ memory `project_auth_scope_2tier` (T11). **BA chốt OUT 4 thay đổi:** Thay đổi 5 (FR-V.II-CROSS-01 + BR-EC-15/16 auto-từ-chối quá hạn / lần 4 bổ sung) · Thay đổi 8 (SLA dynamic "Còn N ngày" + BR-SLA-02 — giữ V3 "4 mức cảnh báo C07") · Thay đổi 12 (BR-FLOW-04 mở rộng — chỉ giữ ngưỡng ≥10 ký tự cho FR-V.II-12) · Thay đổi 13 (Lịch sử thay đổi inline). **2 vấn đề chờ BA chốt** — [`ba-questions-fr06-2026-05-06.md`](../output/qa-reports/round7-2026-05-06/bug-reports/ba-questions-fr06-2026-05-06.md): Q1 hành vi lần 4 (UI disable / BE 4xx / auto TU_CHOI?) chặn CT-006; Q2 SLA "4 mức cảnh báo" ngưỡng % deadline cụ thể chặn CT-021. **Module BLOCKED end-to-end** do thiếu integration LGSP/DVC — R7.6.1 walk full workflow conditional khi BE có inject API hoặc DBA seed bản ghi `CHO_TIEP_NHAN`. R7.7.12.1 smoke regression IMPACT (5 phút × 4 module FR-07/08/11/13) **chạy được ngay** không phụ thuộc LGSP. **Phụ thuộc BE deploy:** schema migration 2 bảng mới THAM_DINH_HO_SO + PHE_DUYET_CHI_TRA + 9 fields lifecycle HSCT + UNIQUE ma_ho_so_dvc + 10 enum CHECK constraint + endpoint `/api/v1/tham-dinh-ho-so` + `/api/v1/phe-duyet-chi-tra`. Cite: [`srs-update-2026-5-5/srs-fr-06-chi-tra.md`](../input/srs-update-2026-5-5/srs-fr-06-chi-tra.md) v3.5 (1.414 dòng) + [`CHANGELOG-v3-to-v3.5.md §srs-fr-06`](../input/srs-update-2026-5-5/CHANGELOG-v3-to-v3.5.md) line 714-866 + [`_DELTA-MAP-FR06.md`](../input/srs-update-2026-5-5/_DELTA-MAP-FR06.md).) · v2.6.6 (2026-05-06 lần 4 — apply SRS update **FR-12 v3.5** đầy đủ stack 8 file QA. **FR-12 v3.5 sync 8 file:** [entity-map.md](../input/data/entity-map.md) +E30 TU_LIEU_PHAP_LY_VV +E31 DANH_GIA_CHAT_LUONG_TV + mở rộng E07 HO_SO_PHAP_LY_DN (định nghĩa chính thức 3.4.3.46 + quyền NHT) + rename E15 TU_VAN_CHUYEN_SAU + bỏ field hinh_thuc_tv + 5 trường công khai + don_vi_id + hop_dong_tv_id; [seed-fixture.yaml v2.7.3](../input/data/seed-fixture.yaml) thêm 3 entity blocks mới (`tv_cs_v35_variants` 7 record cover 7 state, `tu_lieu_phap_ly_vv_variants` 6 record cover 5 enum × 2 state, `danh_gia_chat_luong_tv_variants` 4 record cover idempotent TAO_MOI/CAP_NHAT/GUI_LAI + boundary 1-5); 3 [permission-matrix*.md](../output/permission-matrix.md) bump 52→**55 entity** + 11 row HSPL/TLPL/DGCL × 11 role + NHT 📝 RU* HSPL_DN scoped (Thay đổi 10) + DN 🔌 C† DGCL gửi qua API Cổng PLQG; [funtion/7.12-tu-van-chuyen-sau.md](../output/funtion/7.12-tu-van-chuyen-sau.md) **44→61 TC** (P0:23 P1:34 P2:4) + 17 TC mới TV-045..061 + 4 BR mới (BR-ROUTE-TVCS-01 + BR-PUBLIC-01/02/03) + BR-AUTH-01 đổi 2-tier; [smoke-specs/6.12-smoke-tv-chuyensau.md v2.1](../output/smoke-specs/6.12-smoke-tv-chuyensau.md) đổi tên menu/account `cb_nv_tw_01` + tab Công khai chuyên trang + thanh hành động hàng loạt mở rộng cho DA_DUYET; [test-strategy.md §7.12](../output/test-strategy.md) module rename + Tổng TC project 568→**585** (+17 TVCS v3.5); [tasks/todo.md](todo.md) R7.4.A5 7 state SM-TVCS v3.5 + R7.7.5 61 TC v3.5 spec sync. **13 thay đổi áp:** đổi tên menu (T1) · rename entity (T2) · 7 state SM-TVCS default TIEP_NHAN (T3) · 7 khối Xử lý chi tiết FR-X.1-01 (T4) · 3 entity owned mới (T5) · `don_vi_id` cơ quan tiếp nhận BR-ROUTE-TVCS-01 (T6) · 5 trường công khai BR-PUBLIC-01/02/03 (T7) · 6 khối Xử lý FR-X.1-04/06 (T8) · 3 đầu mối tiếp nhận Cổng PLQG (T9) · NHT 📝 RU* HSPL_DN (T10) · BR-AUTH-01 2-tier bỏ VNPT eKYC (T11) · bỏ field `hinh_thuc_tv` cấp Vụ việc (T12) · link `hop_dong_tv_id` FR-14 (T13). **Phụ thuộc BE deploy:** rename column `noi_dung_tu_van_cs` → `tu_van_chuyen_sau` + 3 bảng mới (HSPL_DN/TLPL_VV/DGCL_TV) + 5 trường công khai + `don_vi_id` + 7 state enum migration + endpoint rename `/api/v1/noi-dung-tu-van-cs` → `/api/v1/tu-van-chuyen-sau`. **Defer:** entity column legacy heuristic trong permission-matrix-by-role.md FR-12 sections (33 row) — bổ sung khi user yêu cầu deep update; OUT Thay đổi 14 (note thang điểm DN đánh giá 0-10 — BA mark OUT giữ nguyên v3 diễn đạt). 2 R7 task FR-12 được update (R7.4.A5 + R7.7.5) — xem [todo.md](todo.md) Phase 4 + Phase 7. Cite: [`srs-update-2026-5-5/srs-fr-12-tv-chuyen-sau.md`](../input/srs-update-2026-5-5/srs-fr-12-tv-chuyen-sau.md) v3.5 + [`CHANGELOG-v3-to-v3.5.md §srs-fr-12`](../input/srs-update-2026-5-5/CHANGELOG-v3-to-v3.5.md) line 1416-1604.) · v2.6.5 (2026-05-06 lần 3 — apply SRS update **FR-05 v3.5** (14 thay đổi IN + V4-CHƯA-SỬA #1) + **FR-12 v3.5** (13 thay đổi IN — 3 entity owned mới HSPL_DN/TU_LIEU_PL/DGCL_TV + rename TVCS + 7 state SM-TVCS). **FR-05 v3.5 sync 6 phase:** Phase 0 verify NotebookLM (notebook id `a4ae45bf` Haizz-HTPLDN, 4 cụm critical match SRS); Phase 1 data layer ([entity-map.md](../input/data/entity-map.md) +E27/E28/E29 PHAN_CONG/DANH_GIA/LICH_SU_VU_VIEC + DON_VI 2 tầng + VU_VIEC bỏ `nguoi_ho_tro_id` thay 3 cột phân công + 5 cột công khai · [seed-fixture.yaml v2.7.2](../input/data/seed-fixture.yaml) SLA VV 10→15 ngày NĐ55 Đ.8 K.1 + 3 variants section mới phan_cong/danh_gia/cong_khai_vu_viec_variants + **YAML structural fix** promote `cap_tai_khoan_cg_nht_r5` ra root level — pre-existing bug từ R7 không do FR-05 update · [flow-module.md §5 SM-VUVIEC](../input/quy-trinh-nghiep-vu/flow-module.md) 12 state + 2 self-loop CONG_KHAI/HUY_CONG_KHAI overlay + Phụ DN bổ sung HS + CB PD từ chối → DANG_XU_LY (KHÔNG TU_CHOI) · [02-thu-tu-module.md §⑥](../input/quy-trinh-nghiep-vu/02-thu-tu-module.md) refactor bảng Transition); Phase 2 SM + smoke ([6.5-sm-vuviec.md](../output/smoke/6.5-sm-vuviec.md) 9→13 test paths · [6.5-smoke-vuviec.md](../output/smoke-specs/6.5-smoke-vuviec.md) 3 smoke optional Công khai/DN bổ sung/filter); Phase 3 test plan FULL ([7.5-vu-viec-htpl.md](../output/funtion/7.5-vu-viec-htpl.md) 33→**72 TC** chia 9 cluster — Cluster 0 base 30 TC + Cluster 1-8 mới 42 TC, P0:44 [2 P0 Critical privacy NĐ 13/2023], P1:27, P2:1; 19→**21 FR** thêm FR-V.I-NEW-02 + FR-V.I-NEW-05); Phase 4 cross-FR delta B (7.4 + 7.4a [4 chỗ fix `nguoi_ho_tro_id`→`nguoi_xu_ly_id`] + 7.4b + 7.7 + 7.10 [BR-AUTH-08 exception TW + QT-027 SLA VV=15] + 7.8 [phân biệt 3 entity đánh giá KE_HOACH/DANH_GIA_VV/DANH_GIA_SAU_VV + flag BR-CALC-04 ID conflict]); Phase 5 impact sample C (7.6 + 7.11 + 7.12 + 7.13 + 7.16 [whitelist 9 fields BR-PUBLIC-04 cho VU_VIEC outbound]); Phase 6 report (3 permission-matrix file + [test-strategy.md v3.2](../output/test-strategy.md) 49→**52 entity**, +2 FR + tasks/todo.md). **2 P0 Critical privacy:** C1-2 (whitelist API outbound chỉ 9 fields, ẨN tên DN/MST/CCCD/SĐT/email/địa chỉ/mo_ta nội bộ/file nghiệp vụ/noi_dung_tu_van — anonymize NQ 03/2017) + C7-2 (SCR-V.I-04 ẩn tên cá nhân CB phụ trách — NĐ 13/2023). **Defer:** SLA NĐ55 Đ.8 K.1 cite chưa web-verify (log bug nếu BE deploy 10 ngày); migration `nguoi_ho_tro_id` cũ (log bug data migration); sandbox VNeID Tier 2 cho TP-Optional-2; BR-CALC-04 ID conflict file 7.5 vs 7.8 (escalate BA renumber). 5 R7 task FR-05 mới (R7.4.A3 update + R7.4.A3-PUBLIC + R7.4.A3-DN-BS + R7.7.3 update + R7.7.3-CL3-PRIVACY) — xem [todo.md](todo.md) Phase 4-7. Tham chiếu: [`_DELTA-MAP-FR05.md`](../input/srs-update-2026-5-5/_DELTA-MAP-FR05.md), [`CHANGELOG-v3-to-v3.5.md §FR-05`](../input/srs-update-2026-5-5/CHANGELOG-v3-to-v3.5.md).) · v2.6.4 (2026-05-06 lần 2 — User chốt 2 thay đổi nghiệp vụ FR-10 sau deep review v2.6.3: **(1) Ngày lễ (FR-VIII-29) gộp tab vào SCR-VIII-06 Cấu hình HT** thay vì màn riêng/DM con — giải quyết DEPLOY-004 UI vị trí; **(2) BỎ HẲN tab "Phân công mặc định" + entity `CAU_HINH_PHAN_CONG` + FR-II-NEW-01 deprecated** — cross-module impact: FR-02 Hỏi đáp / FR-05 Vụ việc / FR-12 TVCS bỏ dropdown gợi ý người xử lý → chuyển sang chọn manual hoặc cơ chế khác (chờ BA verify, OUT-OF-SCOPE FR-10 update đợt này). SCR-VIII-06 còn 4 tab: SLA / Mẫu phản hồi / Quy trình / **Ngày lễ**. Sync 5 file: [`02-thu-tu-module.md`](../input/quy-trinh-nghiep-vu/02-thu-tu-module.md) + [`7.10-quan-tri-he-thong.md`](../output/funtion/7.10-quan-tri-he-thong.md) + [`permission-matrix.md`](../output/permission-matrix.md) (CAU_HINH_PHAN_CONG deprecated 7 row) + [`permission-matrix-by-fr.md`](../output/permission-matrix-by-fr.md) + [`permission-matrix-by-role.md`](../output/permission-matrix-by-role.md) (FR-II-NEW-01 deprecated 11 row + FR-02 entity count 12→11 + FR-10 27→26). 3 câu BA hard block (Q1 NGAY_LE schema / Q2 TINH_THANH UI / Q3 CHO_PHAN_QUYEN scenario) **VẪN cần hỏi** — user 2 thay đổi giải quyết UI vị trí Ngày lễ (DEPLOY-004 close), KHÔNG giải quyết schema/scenario.) · v2.6.3 (2026-05-06 — apply SRS FR-10 v3.5 deep review: 11 inconsistency logged + 10 câu BA (3 hard block: Q1 NGAY_LE schema FR Inputs vs entity 3.4.3.51 / Q2 TINH_THANH UI tab missing / Q3 CHO_PHAN_QUYEN scenario sau bypass; 7 doc clarification: AUDIT_LOG export 10K vs 50K, SLA quá hạn 3 nguồn, SM line 2115 FR-VIII-18 sai, FR-VIII-22 placeholder, FR-VIII-23 actor thiếu DN, SCR-VIII-08a dead UI, SM diagram vs bảng mismatch). Sync 9 file QA: [`bug-report-r7-srs-fr10-inconsistency.md`](../output/qa-reports/round7-2026-05-06/bug-reports/bug-report-r7-srs-fr10-inconsistency.md) (NEW 11 bug + 10 BA Q) + [`smoke/6.10-sm-taikhoan.md`](../output/smoke/6.10-sm-taikhoan.md) (4→5 states + 7→11 transitions) + [`funtion/7.10-quan-tri-he-thong.md`](../output/funtion/7.10-quan-tri-he-thong.md) (25→27 FR + Tier 1/2/3 split — 31 TC active + 6 parking + Tier 3 skip) + [`smoke-specs/6.10-smoke-taikhoan.md`](../output/smoke-specs/6.10-smoke-taikhoan.md) (5 tabs + nút Phân quyền) + [`02-thu-tu-module.md`](../input/quy-trinh-nghiep-vu/02-thu-tu-module.md) (DON_VI 3 cấp → 2 tầng + FR mới 26/28/29 + dead UI SCR-VIII-08a) + [`test-strategy.md §7.10`](../output/test-strategy.md) (25→27 FR + qtht_tw → qtht_01) + [`permission-matrix-by-fr.md §10`](../output/permission-matrix-by-fr.md) (+NGAY_LE row, 9→10 entity) + [`permission-matrix-by-role.md`](../output/permission-matrix-by-role.md) (FR-10 25→27 FR + UC120-123 renumber + 3 row VIII-26/28/29) + [`funtion/7.7-quan-ly-doanh-nghiep.md`](../output/funtion/7.7-quan-ly-doanh-nghiep.md) (3→2 FR, bỏ Import Excel, +DN-019/020). Tier 1 stable test ngay (R7.7.8 Tier 1 31 TC); Tier 2 (FR-VIII-22/26/29) parking chờ BA chốt 3 câu hard block. Test cases chi tiết KHÔNG update theo user yêu cầu — scope chỉ functional plan + execute. Tham chiếu: [`_DELTA-MAP-FR10.md`](../input/srs-update-2026-5-5/_DELTA-MAP-FR10.md), [`CHANGELOG-v3-to-v3.5.md §FR-10 line 1129-1322`](../input/srs-update-2026-5-5/CHANGELOG-v3-to-v3.5.md).) · v2.6.2 (2026-05-06 — sync 8 file QA test plan FR-04 theo SRS update 2026-05-05 + 5 delta map mới/expand. **Mới (4 file):** [`output/funtion/7.4a-nguoi-ho-tro.md`](../output/funtion/7.4a-nguoi-ho-tro.md) (NHT 21 TC) + [`7.4b-to-chuc-tu-van.md`](../output/funtion/7.4b-to-chuc-tu-van.md) (TC TV 25 TC) + [`output/smoke/6.4a-sm-tctv.md`](../output/smoke/6.4a-sm-tctv.md) (SM-TCTV 6 state) + [`6.4b-sm-nht.md`](../output/smoke/6.4b-sm-nht.md) (SM-NHT 4 state). **Rewrite (4 file):** [`7.4-chuyen-gia-tvv.md`](../output/funtion/7.4-chuyen-gia-tvv.md) (18 FR, 30 TC, +CHO_KICH_HOAT, +cong_khai 5 trường, thang 1-5) + [`6.4-sm-tvv.md`](../output/smoke/6.4-sm-tvv.md) (9→**10 state**) + [`output/smoke-specs/6.4-smoke-tvv.md`](../output/smoke-specs/6.4-smoke-tvv.md) (menu "Tư vấn viên / Chuyên gia", URL `/chuyen-gia-tvv/danh-sach`, 6 tab, account `qtht_01`) + [`input/quy-trinh-nghiep-vu/02-thu-tu-module.md`](../input/quy-trinh-nghiep-vu/02-thu-tu-module.md) (fix 6 vị trí sai SRS: TC TV entity riêng, NHT tách entity, SM 10 state, bỏ dia_ban, thang 1-5, dropdown 2 entity). **Permission 3-view sync FR-04:** [`permission-matrix-by-fr.md`](../output/permission-matrix-by-fr.md) (FR-04 mở rộng 3→6 entity row, 49 entity total) + [`permission-matrix-by-role.md`](../output/permission-matrix-by-role.md) (11 role × thêm 7 FR mới = 77 row, mỗi FR-04 block 12→19 row). **5 delta map mới/expand:** `_DELTA-MAP-FR05/06/08/09.md` (mới — cover 14+9+8+6 thay đổi) + `_DELTA-MAP-FR10.md` (expand 3 FR mới → 19 thay đổi đầy đủ). **Defer:** seed-fixture.yaml chưa có 5 trường cong_khai trong tvv_variants/to_chuc_tu_van_variants — bổ sung khi tester chạy R7.2.5/2.6 seed thật. **Fix sai todo R7.4.A1:** "SM 9→11" → "SM 9→**10 state**" (enum SRS line 2011 = 10 values). Tham chiếu: [`_DELTA-MAP-FR04.md`](../input/srs-update-2026-5-5/_DELTA-MAP-FR04.md) §4 File QA cần update.) · v2.6.1 (2026-05-05 — bổ sung R7.0.0 verify scenario reset DB + R7.0.5 re-evaluate 5 R6 task liên quan SRS update. Workflow 3 bước trigger Round 7: (1) hỏi dev scenario A/B/C trước deploy; (2) sau deploy QA chạy 3 curl verify schema + entity NGUOI_HO_TRO + count DN cũ; (3) phân loại scenario thực tế → adjust scope. Scenario A=no reset (resume R6 song song R7), B=reset DB (frozen R6 + gộp R7), C=partial (STOP escalate dev). 5 R6 task có liên quan: R6.5.1 KPI-07, R6.5.3 SLA, R6.7.2 CG/TVV 31 TC enum, R6.7.4 DN 8 TC flow, R6.4.A1.5 PC TVV dropdown — KHÔNG đơn giản gộp vào R7, phải re-evaluate per scenario.) · v2.6.0 (2026-05-05 — apply 3 SRS update từ `input/srs-update-2026-5-5/` (FR-04 NHT/TVV/TCTV + FR-07 DN self-reg + FR-10 mở rộng). Mở thêm **Round 7 PENDING** sau Round 6 close — 30 task chia 8 phase. Scope mở rộng 46→49 entity (thêm NGUOI_HO_TRO + TO_CHUC_TU_VAN + NGAY_LE) + 9 FR mới (3 FR-10: VIII-26/28/29 + 6 FR-04: IV-13/NHT-01/02/03 + IV-NEW-01/02/04) - 1 FR bỏ (FR-V.III-NEW-01 Import DN). Fixture bump v2.6.2 → v2.7.0 thêm 3 block (ngay_le_variants + to_chuc_tu_van_variants + nht_variants). 5 file QA artifact đã sync delta (entity-map + permission-matrix + flow-module + seed-fixture + test-strategy). 3 file `_DELTA-MAP-*.md` chứa impact analysis 16 module + đáp án 6/11 câu BA + 4 open issue defer khi test. Tham chiếu: [`_DELTA-MAP-FR04.md`](../input/srs-update-2026-5-5/_DELTA-MAP-FR04.md), [`_DELTA-MAP-FR07.md`](../input/srs-update-2026-5-5/_DELTA-MAP-FR07.md), [`_DELTA-MAP-FR10.md`](../input/srs-update-2026-5-5/_DELTA-MAP-FR10.md). Round 7 task list: [`todo.md` §Round 7](todo.md#round-7--pending-apply-3-srs-update-từ-srs-update-2026-5-5).) · v2.5.1 (2026-05-02 — bump scope Round 6 từ 5 phase → 7 phase: thêm Phase 6 Workflow đầu ra hậu kỳ (mapping R5 P3 — Chi trả/TV nhanh/CT HTPLDN GĐ1+GĐ2) + Phase 7 Functional 17 module (mapping R5 T4.1-T4.17). Lý do: R6 ban đầu cắt scope so với R5; user yêu cầu update để phản ánh đúng work còn lại. Tổng task R6 từ 52 → 74. Chi tiết [`output/qa-reports/round6-2026-05-01-postreset/README.md`](../output/qa-reports/round6-2026-05-01-postreset/README.md) Phase 6+7.) · v2.5 (2026-05-01 — bump sau dev reset DB 1/5: scope 6 phase + 16 module GIỮ NGUYÊN, mở thêm **Round 6** cho post-reset re-seed. Round 5 frozen làm reference lịch sử bug 5 tuần. Fixture bump v2.6.2 thêm `cap_tai_khoan_cg_nht_r5` + `dn_variants_dp_extension`.) · v2.4 (sync count refs với fixture v2.5 — pool 12 DN/12 TVV/15 VV/11 HD/10 TVCS/11 BM)
...
| **E3** | Chi trả (FR-06) ✏️ v3.5 | 🚫 chờ LGSP đồng bộ + BA chốt Q1+Q2 | Daily verify SCR-V.II-01 list count + 5 tab phân loại "Tất cả/Chờ xử lý/Đang đánh giá/Chờ PD/Đã xử lý" + verify negative "không có nút Thêm mới" + 10 trạng thái Việt + UI badge tiếng Việt thuần. R7.7.12.1 smoke regression IMPACT (FR-07/08/11/13 × 5 phút) chạy được ngay |
